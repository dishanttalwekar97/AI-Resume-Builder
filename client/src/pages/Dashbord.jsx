import { FilePenLineIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloud, XIcon, CloudIcon, LoaderCircleIcon } from 'lucide-react'
...
const Dashbord = () => {
  ...
  const colors = ["#9333ea", "#d97706", "#dc2626", "#0284c7", "#1634af"] 
  const [allResumes, setAllResumes] = useState([])
  const [showCerateResume, setShowCreateResume] = useState(false) 
  ...
  const deletResume = async (resumeId) =>{
   try {
     const confirm = window.confirm('Are you sure you ant to delete this resume?')
    if(confirm){
    ...
    <div>
      ...
      <div className='max-w-7xl mx-auto px-4 py-8'>
        <p className='text-2xl font-medium mb-6 bg-gradient-to-r from-slate-600 to-slate-700 
          bg-clip-text text-transparent sm:hidden'>Welcome, Dishant talwekar</p>
        <div className='flex gap-4'>
          <button onClick={() => setShowCreateResume(true)}
            ...
            <PlusIcon className='size-11 transition-all duration-300 p-2.5
              bg-gradient-to-r from-indigo-300 to-indigo-500 text-white rounded-full' /> 
          ...
          <button onClick={() => { setshowUploadResume(true) }} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300
            group hover:border-purple-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
            <CloudIcon className='size-11 transition-all duration-300 p-2.5
            ...
            return (
              <button key={index} onClick={()=> navigate(`/app/builder/${resume._id}`)} className='relative w-full sm:max-w-36 h-48 flex
                ...
                <p className='absolute bottom-1 text-[11px] text-slate-400
                 group-hover:text-slate-500 transition-all duration-300 px-2 text-center' style={{ color: baseColor + '90' }}> 
...
hover:text-slate-600 cursor-pointer transition-colors' onClick={() => { setShowCreateResume(false); setTitle('') }} /> 
            </div>
          </form>
        )}
